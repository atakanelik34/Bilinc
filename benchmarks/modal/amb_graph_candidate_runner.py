#!/usr/bin/env python3
"""Matched Modal A/B runner for Bilinc's opt-in bounded graph recall signal."""
...
BENCHMARK_ID = "amb-legacy-v3-graph-candidate"
...
app = modal.App(
    f"bilinc-benchmark-{GOAL_ID}-amb-graph",
    tags={
        "project": "bilinc",
        "purpose": "benchmark",
        "goal": GOAL_ID,
...
def _run_layer(layer: int, graph_enabled: bool, store_delay_seconds: float) -> dict[str, Any]:
    os.environ.pop("BILINC_GRAPH_RECALL", None)
    if graph_enabled:
        os.environ["BILINC_GRAPH_RECALL"] = "1"

    label = "candidate_graph" if graph_enabled else "baseline_disabled"
...
def run_amb_pair(store_delay_seconds: float = 3.0) -> dict[str, Any]:
    """Run both AMB layers with baseline and candidate in one container."""
    results: dict[str, dict[str, Any]] = {}
    for layer in (1, 2):
        for label, enabled in (("baseline_disabled", False), ("candidate_graph", True)):
    ...
    return {
        ...
        "product_files": {
            ...
            "src/bilinc/core/knowledge_graph.py": _sha256(REMOTE_REPO / "src/bilinc/core/knowledge_graph.py"),
            "benchmarks/adapters/amb_generic_mcp.py": _sha256(REMOTE_REPO / "benchmarks/adapters/amb_generic_mcp.py"),
            "benchmarks/modal/amb_graph_candidate_runner.py": _sha256(
                REMOTE_REPO / "benchmarks/modal/amb_graph_candidate_runner.py"
            ),
        },
