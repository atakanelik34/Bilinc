...
def test_public_api_is_cloud_only():
    ...
    assert bilinc.__version__ == "2.2.0"
    assert bilinc.version == "2.2.0"
    assert "utm_campaign=activation_2_2_0" in bilinc.ACTIVATION_SIGNUP_URL
    ...
    assert transport.calls == [
        {
            ...
            "headers": {
                ...
                "User-Agent": "bilinc-python/2.2.0",
                "X-Bilinc-Client": "python-sdk",
                "X-Bilinc-Client-Version": "2.2.0",
            },
            "body": json.dumps(
                {
                    "key": "project.status",
...
def test_cli_signup_and_missing_key_failure(monkeypatch, capsys):
    ...
    assert "activation_2_2_0" in signup_out.out
...
def test_cli_start_login_doctor_quicktest_and_mcp_config(monkeypatch, capsys, tmp_path):
    ...
    assert "activation_2_2_0" in start_out.out
