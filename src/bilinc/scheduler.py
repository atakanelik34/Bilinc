"""Source-only Phase 7 background scheduler and job orchestration.

This module supports the internal StatePlane runtime. It is intentionally
excluded from the public cloud-only wheel and sdist; callers importing it must
run from the source/runtime distribution.
"""
...
class BackgroundScheduler:
    """Simple async scheduler for source-only periodic and trigger-based jobs."""
    ...
    def register_phase7_jobs(self) -> None:
        """Register canonical internal-runtime periodic jobs."""
    ...
    async def run_trigger_checks(self) -> Dict[str, Any]:
        """Consolidate immediately when working-memory capacity is high."""
    ...
    async def start(self, tick_seconds: float = 5.0) -> None:
        """Start the periodic scheduler loop as a background task."""
