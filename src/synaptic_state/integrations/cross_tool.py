...
- This layer provides universal translation via the SynapticAI StatePlane

Architecture:
1. Source Format Parser (Claude → SynapticAI)
2. StatePlane Storage (unified representation)
3. Target Format Generator (SynapticAI → Cursor, etc.)
...
from synaptic_state.core.models import MemoryEntry, MemoryType
...
class CrossToolTranslator:
    ...
    def import_from_tool(
        ...
        for block in blocks:
            result = self.state_plane.commit(
                key=f"{tool_format.value}:{block.source_path}:{hash(block.content) % 10000}",
                value=block.content,
                memory_type=block.memory_type,
                metadata=block.metadata,
                source=tool_format.value,
                session_id=session_id,
                verify=verify,
            )
            committed_keys.append(result.get("key", ""))
    ...
    def export_to_tool(
        ...
        result = self.state_plane.recall(
            intent=f"export to {tool_format.value}",
        ...
        output = exporter(result.memories)
        
        if path and overwrite:
            os.makedirs(os.path.dirname(path) if os.path.dirname(path) else ".", exist_ok=True)
            with open(path, "w") as f:
                f.write(output)
            logger.info(f"Exported {len(result.memories)} memories to {path}")
    ...
    def _export_to_claude_code(self, memories: List[MemoryEntry]) -> str:
        ...
        for m in memories:
            if m.memory_type == MemoryType.SYMBOLIC:
        ...
        output = "# CLAUDE.md (Generated by SynapticAI)\n"
    ...
    def _export_to_openclaw(self, memories: List[MemoryEntry]) -> str:
        ...
        output = "# AGENTS.md (Generated by SynapticAI)\n"
        output += f"# SynapticAI State Plane Export\n\n"
    ...
    def _export_to_generic(self, memories: List[MemoryEntry]) -> str:
        ...
        lines = ["# SynapticAI Memory Export\n"]
        for m in memories:
